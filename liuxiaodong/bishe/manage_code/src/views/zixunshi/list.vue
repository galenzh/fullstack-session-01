<template>
	<div>
			...
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.zixunshixingming" placeholder="咨询师姓名"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<div class="btn_view">
					<el-button class="add_btn" type="success" @click="addClick" v-if="btnAuth('zixunshi','新增')">
						新增
					</el-button>
				</div>
			</div>
			<div class="dataList"
				 v-if="btnAuth('zixunshi','查看')"
			>
				<div class="item" v-for="(item,index) in list" :key="item.id">
					...
					<div class="title">咨询师账号：{{item.zixunshizhanghao}}</div>
					<div class="title">咨询师姓名：{{item.zixunshixingming}}</div>
                    <div class="title">
					...
					<div class="btns">
						<el-button class="view_btn" type="info" v-if=" btnAuth('zixunshi','查看')" @click="infoClick(item.id)">
							详情
						</el-button>
						<el-button class="edit_btn" type="primary" @click="editClick(item.id)" v-if=" btnAuth('zixunshi','修改')">
							修改
						</el-button>
						<el-button class="del_btn" type="danger" @click="delClick(item.id)"  v-if="btnAuth('zixunshi','删除')">
							删除
						</el-button>
						<el-button class="cross_btn" v-if="btnAuth('zixunshi','预约')" type="success" @click="zixunshiyuyueCrossAddOrUpdateHandler(item,'cross','是','','','')">
							预约
						</el-button>
                        <el-button class="operate_btn" v-if="item.sfsh!='是' && btnAuth('zixunshi','审核')" @click="approvalClick(item)" type="warning">审核</el-button>
                        <el-button class="operate_btn" v-if="btnAuth('zixunshi','私信')" type="success" @click="chatClick(item)">
                          私信
                        </el-button>
					</div>
				</div>
		...
		<zixunshiyuyueFormModel ref="zixunshiyuyueFormModelRef" @formModelChange="formModelChange"></zixunshiyuyueFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.zixunshixingming&&searchQuery.value.zixunshixingming!=''){
			params['zixunshixingming'] = '%' + searchQuery.value.zixunshixingming + '%'
		}
	...
	const approvalClick = (row) => {
		let params = {
			...
			mima: row.mima,
			zixunshixingming: row.zixunshixingming,
			xingbie: row.xingbie,
			touxiang: row.touxiang,
			lianxifangshi: row.lianxifangshi,
			zizhirenzheng: row.zizhirenzheng,
	...
	import zixunshiyuyueFormModel from '@/views/zixunshiyuyue/formModel'
	const zixunshiyuyueFormModelRef = ref(null)
    const zixunshiyuyueCrossAddOrUpdateHandler = (row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			zixunshiyuyueFormModelRef.value.init(row.id,'cross','预约',row,'zixunshi',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
    ...
    const chatClick = (row) => {
        ...
        if (row.zixunshizhanghao) {
            nowname.value = row.zixunshizhanghao
        }
