<template>
	<div class="detail-page" :style='{}'>
			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.kechengmingcheng}}</span>
					</div>
		...
		<div class="video_view">
			<video v-if="detail.kechengshipin" :src="$config.url + detail.kechengshipin" controls></video>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            bannerList.value = res.data.data.kechengtupian?res.data.data.kechengtupian.split(','):[]
	...
	const collectClick = (type) => {
		if (type == 1 && !collectType.value) {
			let params = {
				...
				inteltype: detail.value.kechengmingcheng,
	...
	const addCart = () => {
		...
		cartForm.value.goodtype = detail.value.kechengmingcheng
	...
	const buyNow = () => {
		let data = {
			...
			goodtype: detail.value.kechengmingcheng,
			picture: bannerList.value[0],
	...
	const jfTap = () => {
		let data = {
			...
			buynumber: buyNumber.value,
			userid: context?.$toolUtil.storageGet('userid'),
			price: detail.value.jf,
			discountprice: detail.value.jf ?detail.value.jf : 0,
			goodtype: detail.value.kechengmingcheng,
			picture: bannerList.value[0],
		}
		context?.$toolUtil.storageSet('orders_good',JSON.stringify([data]))
