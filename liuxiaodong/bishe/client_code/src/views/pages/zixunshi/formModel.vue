			...
			<div class="info_view">
				<div class="title_view">
					<div class="detail_title">
						<span>{{detail.zixunshizhanghao}}</span>
					</div>
				...
				<div class="btn_view">
					<el-button class="edit_btn" type="primary" v-if="btnAuth('zixunshi','私信')&&detail.id!=user.id" @click="chatClick">私信</el-button>
					<el-button class="cross_btn" v-if="btnFrontAuth('zixunshi','预约')" @click="zixunshiyuyueonAcross('预约','是','','','')" type="warning">预约</el-button>
					<el-button class="approval_btn" v-if="btnAuth('zixunshi','审核')" type="warning" @click="approvalClick()">审核</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('zixunshi','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('zixunshi','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const getDetail = () => {
		context?.$http({
            ...
            title.value = res.data.data.zixunshizhanghao
			if(res.data.data.touxiang){
				fpic.value = res.data.data.touxiang.split(',')[0]
			}else if(res.data.data.headportrait){
				fpic.value = res.data.data.headportrait.split(',')[0]
			}
            if(res.data.data.zixunshizhanghao){
                fname.value = res.data.data.zixunshizhanghao
            }
	...
	const zixunshiyuyueonAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		...
		nextTick(()=>{
			router.push(`/index/zixunshiyuyueAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	const fid = ref('')
