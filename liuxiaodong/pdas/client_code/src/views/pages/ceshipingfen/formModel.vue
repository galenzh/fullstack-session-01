	...
	<div class="detail-page" :style='{}'>
		...
		<div class="detail_view">
			...
			<div class="info_view">
				...
				<div class="info_item">
					<div class="info_label">评分时间</div>
					<div  class="info_text" >{{detail.pingfenshijian}}</div>
				</div>
				<div class="info_item">
				...
				<div class="info_item">
					<div class="info_label">评分备注</div>
					<div  class="info_text" >{{detail.pingfenbeizhu}}</div>
				</div>
				<div class="info_item">
					<div class="info_label">测试报告</div>
					<div class="info_text">
						<el-button class="info_down" v-if="detail.ceshibaogao" @click="downClick(detail.ceshibaogao)">点击下载</el-button>
						<el-button class="info_undown" v-else>暂无</el-button>
					</div>
				</div>
				<div class="btn_view">
					<el-button class="cross_btn" v-if="btnFrontAuth('ceshipingfen','建议')" @click="jiankangjianyionAcross('建议','','','','')" type="warning">建议</el-button>
					<el-button class="edit_btn" v-if="centerType&&btnAuth('ceshipingfen','修改')" type="primary" @click="editClick">修改</el-button>
					<el-button class="del_btn" v-if="centerType&&btnAuth('ceshipingfen','删除')" type="danger" @click="delClick">删除</el-button>
				</div>
			</div>
		</div>
...
<script setup>
	...
	const jiankangjianyionAcross = async (btnType,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1) => {
		if(!context?.$toolUtil.storageGet('frontToken')){
			context?.$toolUtil.message('请登录后再操作！','error')
			return false
		}
		if(!btnAuth('ceshipingfen',btnType)){
		...
		nextTick(()=>{
			router.push(`/index/jiankangjianyiAdd?type=cross&&id=${detail.value.id}`)
		})
	}
	onMounted(()=>{
		init()
