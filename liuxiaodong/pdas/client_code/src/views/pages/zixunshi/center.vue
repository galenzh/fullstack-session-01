		...
		<div class="usersView">
			...
			<div class="usersBox updateInfo" v-if="tabIndex=='center'">
				<el-form class="usersForm" ref="userFormRef" :model="userForm" label-width="120px" :rules="rules">
					<el-row>
						<el-col :span="12">
							<el-form-item prop="zixunshizhanghao" label="咨询师账号">
								<el-input class="list_inp" v-model="userForm.zixunshizhanghao" placeholder="咨询师账号" readonly></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item prop="zixunshixingming" label="咨询师姓名">
								<el-input class="list_inp" v-model="userForm.zixunshixingming" placeholder="咨询师姓名" ></el-input>
							</el-form-item>
						</el-col>
						<el-col :span="12">
						...
						<el-col :span="24">
							<el-form-item prop="zizhirenzheng" label="资质认证">
								<uploads
									action="file/upload" 
									tip="请上传资质认证"
									style="width: 100%;text-align: left;"
									:fileUrls="userForm.zizhirenzheng?userForm.zizhirenzheng:''" 
									@change="zizhirenzhengUploadSuccess">
								</uploads>
							</el-form-item>
						</el-col>
						<el-col :span="12">
							<el-form-item prop="storeupNumber" label="收藏数">
								<el-input class="list_inp" v-model="userForm.storeupNumber" placeholder="收藏数" ></el-input>
							</el-form-item>
						</el-col>
					</el-row>
...
<script setup>
	...
	const getSession = () =>{
		context?.$http({
			...
			context?.$toolUtil.storageSet("frontName", res.data.data.zixunshizhanghao)
	...
	//资质认证上传回调
	const zizhirenzhengUploadSuccess=(e)=>{
		userForm.value.zizhirenzheng = e
	}
	//初始化
	const init = () => {
	...
	const updateSession = () => {
		userFormRef.value.validate((valid)=>{
			if(valid){
				...
				if(userForm.value.zizhirenzheng!=null){
					userForm.value.zizhirenzheng = userForm.value.zizhirenzheng.replace(new RegExp(context?.$config.url,"g"),"");
				}
				store.dispatch('user/update',userForm.value).then(res=>{
					if(res?.data&&res.data.code==0){
						context?.$toolUtil.message('更新成功','success')
