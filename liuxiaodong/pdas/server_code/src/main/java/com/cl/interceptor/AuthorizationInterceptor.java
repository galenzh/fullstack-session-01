...
public class AuthorizationInterceptor implements HandlerInterceptor {
	...
	if (request.getMethod().equals(RequestMethod.OPTIONS.name())) {
        ...
        if(tokenEntity != null) {
        	...
        	// 在权限判断中添加科室主任的判断
        	if("keshizhureng".equals(tokenEntity.getRole())) {
        	    // 科室主任权限控制
        	    if(tokenEntity.getTablename().equals("keshizhureng")) {
        	        Long userId = (Long)request.getSession().getAttribute("userId");
        	        if(userId != null) {
        	            KeshizhurengEntity user = keshizhurengService.selectById(userId);
        	            if(user != null) {
        	                return true;
        	            }
        	        }
        	    }
        	    // 可以在这里添加科室主任可以访问的其他表的权限
        	}
